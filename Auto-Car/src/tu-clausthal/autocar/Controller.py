...
class Controller:
    ...
    def toggle_ignore_logic_button(self):
        ...
        if self.ignoreLogic:
            ...
            self.gui.ignore_logic_set_color("green")
        else:
            print("Stopped ignoring Logic")
            self.gui.ignore_logic_set_color("red")
        return
    ...
    def connect_to_car(self):
        if "Mobilitaetslabor" in str(subprocess.check_output("netsh wlan show interfaces")):
            self._mqtt_connection.connect("192.168.50.141")
        else:
