...
class Controller:
    ...
    def on_window_close(self):
        """
        stops timer and simulator if window is closed
        """
        if self.rep_timer:
            self.rep_timer.stop()
        Simulator.stop()
    ...
    def subscribe(self):
        """
        subscribes to mqtt data (rc, sensor, lidar)
        """
    ...
    def on_connect(self, client, userdata, flags, rc):
        """
        if successfully connected, subscribe and set connect button color to green
        """
        self.subscribe()
    ...
    def on_disconnect(self, client, userdata, rc):
        """
        if successfully disconnected, set connect button color to default
        """
    ...
    def on_data_change(self, data_cahnged_str):
        """
        checks if sensor data has changed and updates position and headinglabel
        """
    ...
    def start_cmd_mosq_path(self):
        """
        used only in advanced debugging GUI
        """
    ...
    def on_speed_change_scale(self, val):
        """
        sets speed to 90-current slider value
        """
    ...
    def on_steer_change_scale(self, val):
        """
        sets steer to 90+current slider value
        """
    ...
    def stop_cmd_btn(self):
        """
        if pressed stops the car, if stop is active sets stop button color to red, else sets it so default
        """
    ...
    def reset_map_btn(self):
        """
        deletes all points in the occupancy grid
        """
    ...
    def reset_euler_btn(self):
        """
        sets current euler data to new calculated offset
        """
    ...
    def toggle_manual_control_button(self):
        """
        if manual control is activated sets button color to red
        if autonomous driving is activated set color to default
        """
        # disable autopilot
    ...
    def toggle_autopilot_button(self):
        """
        if autopilot is activated sets button color to red
        if autopilot is disabled sets color to default
        """
    ...
    def connect_to_car_btn(self):
        """
        checks if device is connected to same wifi as car
        """
    ...
    def show_autopilot_speed_steer(self, speed, steer):
        """
        displays current steer and speed
        """
    ...
    def record_btn(self):
        """
        starts recording and saves it as .txt
        """
    ...
    def play_btn(self):
        """
        simulates selected recorded data
        """
